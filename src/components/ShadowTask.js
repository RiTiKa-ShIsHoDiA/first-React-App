...
function ShadowTask({listType,setForm}) {
  ...
  return (
    <div id = "shadowoftask" className= {listType ? ` w-full h-40 ${showtask}`: ` w-60 h-56 ${showtask}`}>
      <p className='text-slate-500 cursor-pointer dark:text-white ' onClick = {(e)=>setForm(true)}>Add new task</p>
    </div>
  )
}
